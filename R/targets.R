...
get_fcpg = function(base = 'https://prod-is-usgs-sb-prod-publish.s3.amazonaws.com/5f3ab12082ce8df5b6c4076a/params.json'){

  o = read_json(base, simplifyVector = TRUE)

  meta = matrix(c('lc1',  'Percent upstream Temperate or sub-polar needleleaf forest', '%',
    ...
    'fac',  'An integer flow accumulation grid which contains the number of cells within CONUS draining to each cell within CONUS based on the HydroDEM', ""), ncol = 3, byrow = T) %>%
    data.frame() %>%
    setNames(c("variable", "description", 'units'))

   data.frame(id = "USGS FCPG", varname = o$parameters, URL =  unlist(o$urls)) %>%
    mutate(variable = gsub("_fcpg.vrt", "", basename(URL))) %>%
    vrt_meta() %>%
    mutate(type = "vrt", tiled = "") %>%
